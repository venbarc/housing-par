...
import { ArrestCount, Bed, DaysRange, PastYearCount, Patient, ServiceAccess, YesNoNA } from '../../types';
...
export default function PatientCreateForm({ beds }: Props) {
    const availableBeds = beds.filter((bed) => (bed.patients?.length ?? 0) === 0 && bed.status !== 'maintenance');
    const form = useForm({
        first_name: '',
        last_name: '',
        dob: '',
    ...
    const submit = (event: FormEvent) => {
        ...
        form.transform((data) => ({
            ...
            bed_id: data.bed_id ? Number(data.bed_id) : (null as any),
        ...
        form.post('/patients', {
            ...
            onError: (errors) => toast.error(String(Object.values(errors)[0] ?? 'Could not create intake record.')),
        });
    };
    ...
    return (
        <form onSubmit={submit} className="card p-4">
            <div className="mb-3">
                <h3 className="text-lg font-bold">Intake Person</h3>
                <p className="text-sm text-slate-500">Create an intake record and optional bed assignment</p>
            </div>

            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div>
                    <label className="field-label">First Name</label>
                    <input className="form-input" value={form.data.first_name} onChange={(e) => form.setData('first_name', e.target.value)} required />
                </div>
                <div>
                    <label className="field-label">Last Name</label>
                    <input className="form-input" value={form.data.last_name} onChange={(e) => form.setData('last_name', e.target.value)} required />
                </div>
                <div>
                    <label className="field-label">DOB</label>
                    <input type="date" className="form-input" value={form.data.dob} onChange={(e) => form.setData('dob', e.target.value)} required />
                </div>
                <div>
                ...
                <div>
                    ...
                    <input
                        ...
                        disabled={form.data.status === 'walk_in'}
                ...
                <div>
                    ...
                    <input className="form-input" value={form.data.insurance} onChange={(e) => form.setData('insurance', e.target.value)} />
                </div>
                <div>
                    <label className="field-label">Intake Date</label>
                    <input type="date" className="form-input" value={form.data.intake_date} onChange={(e) => form.setData('intake_date', e.target.value)} required />
                </div>
                <div>
                    <label className="field-label">Discharge Date</label>
                    <input type="date" className="form-input" value={form.data.discharge_date} onChange={(e) => form.setData('discharge_date', e.target.value)} />
                </div>
                ...
                <div className="sm:col-span-2">
                    ...
                    <select
                        ...
                        onChange={(e) => form.setData('bed_id', e.target.value ? Number(e.target.value) : null)}
                    >
                        <option value="">No bed yet</option>
                        {availableBeds.map((bed) => {
                            const facilityName = bed.room?.facility?.name ?? 'Name not set';
                            const roomName = bed.room?.name ?? 'No room';
                            const label = `${facilityName} • ${roomName} • Bed ${bed.bed_number} (${bedTypeLabel(bed.bed_type)})`;
                            return (
                                <option key={bed.id} value={bed.id}>{label}</option>
                            );
                        })}
                    </select>
                </div>
            ...
            <div className="mt-4 rounded-xl border border-[color:var(--border-soft)] bg-[color:var(--surface-soft)] p-4">
                ...
                <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            required
                            disabled={form.data.is_veteran === '0'}
                        >
                            <option value="">Select</option>
